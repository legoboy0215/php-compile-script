...
#This is for 1.6.23 or older. Uncomment this if you are going to compile with 1.6.23 or older
#download_file "https://sourceforge.net/projects/libpng/files/libpng16/older-releases/$LIBPNG_VERSION/libpng-$LIBPNG_VERSION.tar.gz" | tar -zx >> "$DIR/install.log" 2>&1

#This is for 1.6.24 or newer. Comment this if you are going to compile with 1.6.23 or older
...
if [ "$IS_CROSSCOMPILE" != "yes" ] && [ "$DO_STATIC" == "no" ]; then
	...
	echo "zend_extension=$DIR/bin/php7/lib/php/extensions/no-debug-zts-20151012/opcache.so" >> "$DIR/bin/php7/bin/php.ini"
	...
	echo ";Remove the ; if you want have errors with using localhost with MySQLi" >> "$DIR/bin/php7/bin/php.ini"
	echo ";mysqli.default_socket=/var/run/mysqld/mysqld.sock" >> "$DIR/bin/php7/bin/php.ini"
fi
...
date >> "$DIR/install.log" 2>&1
echo " done!"
echo "[PocketMine] You should be able start the server now using \"./start.sh.\""
echo "[PocketMine] If it doesn't work, please send the \"install.log\" file to the Issue Tracker."
